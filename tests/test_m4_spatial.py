...
class M4SpatialTests(unittest.TestCase):
    ...
    def test_constant_field_has_no_spread(self):
        ...
        self.s.set(optics_on=1,halation_amount=100,bloom_amount=100,glow_amount=100,veil_amount=80,
            halation_threshold=-20,bloom_threshold=-20,glow_threshold=-20,veil_threshold=-20)
    ...
    def test_impulse_spreads(self):
        ...
        self.s.set(optics_on=1,optics_quality=1,bloom_amount=100,bloom_radius=36,bloom_threshold=-20,optics_soft=50)
        out=self.s.render_image(data,w,h)
    ...
    def test_quality_tiers_deterministic_finite(self):
        ...
        for q in [0,1,2]:
            self.s.set(optics_on=1,optics_quality=q,bloom_amount=70,glow_amount=40,halation_amount=50,veil_amount=15,
                bloom_threshold=-10,glow_threshold=-10,halation_threshold=-10,veil_threshold=-10)
    ...
    def test_zero_amount_path_matches_optics_off(self):
        ...
        self.s.set(optics_on=1,optics_quality=2,optics_mix=2,optics_soft=200,halation_radius=110,bloom_radius=170,glow_radius=290,veil_radius=580)
        a=self.s.render_image(data,w,h); self.s.set(optics_on=0); b=self.s.render_image(data,w,h)
        for x,y in zip(a,b): self.assertTrue(math.isclose(x,y,rel_tol=2e-6,abs_tol=2e-6))
    def test_contribution_view_guard(self):
        self.s.set(optics_on=1,bloom_amount=100,optics_view=2,output_mode=1)
